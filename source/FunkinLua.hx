...
class FunkinLua
{
	...
	public function new(script:String)
	{
		...
		set('modchartsDisabled', PlayState.instance.modchartsDisabled);
		...
		{
			PlayState.instance.addShaderToCamera(camera, new Shaders.GlitchEffect(waveSpeed, waveFrq, waveAmp));
		});
		...
		{
			PlayState.instance.addShaderToCamera(camera, new Shaders.PulseEffect(waveSpeed, waveFrq, waveAmp));
		});
		...
		{
			PlayState.instance.addShaderToCamera(camera, new Shaders.DistortBGEffect(waveSpeed, waveFrq, waveAmp));
		});
		Lua_helper.add_callback(lua, "addInvertEffect", function(camera:String, lockAlpha:Bool = false)
		{
			PlayState.instance.addShaderToCamera(camera, new Shaders.InvertColorsEffect(lockAlpha));
		});
		...
		{
			if (PlayState.instance.modchartsDisabled && variable.toLowerCase().contains("cam"))
				return false;
			var killMe:Array<String> = variable.split('.');
			if (killMe.length > 1)
			{
		...
		Lua_helper.add_callback(lua, "setPropertyFromClass", function(classVar:String, variable:String, value:Dynamic)
		{
			if (PlayState.instance.modchartsDisabled
				&& (classVar.toLowerCase().contains("flxg") || classVar.toLowerCase().contains("lib")))
				return false;
			@:privateAccess
			var killMe:Array<String> = variable.split('.');
			if (killMe.length > 1)
