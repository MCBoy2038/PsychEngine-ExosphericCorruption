...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		{ // Dialogue ending
			...
			if (supercoolbadassimage != null) // might be bad if done in a row
			{
				remove(supercoolbadassimage, true);
				supercoolbadassimage.destroy();
				supercoolbadassimage = null;
			}
			if (bgFade != null)
			{
				bgFade.alpha -= 0.5 * elapsed;
	...
	var supercoolbadassimage:FlxSprite;
	...
	function startNextDialog():Void
	{
		...
		if (curDialogue.text.startsWith("[image|"))
		{
			var imageName = curDialogue.text.split("|")[1].replace("]", " ").trim();
			supercoolbadassimage = new FlxSprite().loadGraphic(Paths.image("dialogue/" + imageName));
			supercoolbadassimage.setGraphicSize(Std.int(box.width * 0.89), Std.int(box.height * 0.55));
			supercoolbadassimage.updateHitbox();
			supercoolbadassimage.setPosition(box.x + (box.width - supercoolbadassimage.width - 44), box.y + (box.height - supercoolbadassimage.height - 26));
			add(supercoolbadassimage);
			curDialogue.text = "";
		}
