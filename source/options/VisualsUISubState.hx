...
class VisualsUISubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Lane Transparency', 'Sets visibility of lane transparency.', 'underlaneVisibility', 'percent', 0);
		addOption(option);
		...
		var option:Option = new Option('Opponent Lane Transparency', 'Sets visibility of opponent\'s lane transparency.', 'opponentUnderlaneVisibility',
			'percent', 0);
		addOption(option);
		option.scrollSpeed = 1;
		option.minValue = 0.0;
		...
		var option:Option = new Option('Pause Screen Song:', "What song do you prefer for the Pause Screen?", 'pauseMusic', 'string', 'Tea Time',
			['None', 'Breakfast', 'Tea Time', 'Take a Break']);
		addOption(option);
